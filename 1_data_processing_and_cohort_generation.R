...
# dhb #### - do this first thsi is the population of interest
...
# "centering" (actaully shifting) deprivation ####
...
#which dates was the cohort indexed on
...
tchdl = tchdl_ %>%
  ...
  select(VSIMPLE_INDEX_MASTER,FACILITY_CODE,RESULT_DATE,test,result) 
tri = tri_ %>%
  mutate(test = "TG",result = EN_OBSR_RESULT_NUM) %>%
  select(VSIMPLE_INDEX_MASTER,FACILITY_CODE,RESULT_DATE,test,result)
...
#8. Join tests together
#bind results together
all_tests = rbind(hba1c,egfr,tchdl,tri)
dim(all_tests)

#9. remove results from after an individual had an event
temp = hc_dt[,c("VSIMPLE_INDEX_MASTER","indicator","total_fu","end_fu_date_event","end_fu_date")]
event_exclude = temp[indicator == 1,c("VSIMPLE_INDEX_MASTER","end_fu_date_event")]

all_tests = as.data.table(all_tests)

# event_exclude
clean_tests = event_exclude[
  all_tests,on = "VSIMPLE_INDEX_MASTER"][
    ,post_event := end_fu_date_event < RESULT_DATE
  ][
    post_event == FALSE | is.na(post_event),
  ][
    ,c("VSIMPLE_INDEX_MASTER","RESULT_DATE","test","result")
  ]
# clean_tests
# dim(clean_tests)
...
egfr_exclusions = clean_tests %>%
  filter(test == "eGFR",
             RESULT_DATE >= as.Date("2007-12-31"),
...
write.fst(clean_tests,
          path = "data/testsafe/clean_tests.fst")

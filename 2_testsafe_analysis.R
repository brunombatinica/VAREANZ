...
# 1. Raw test explorations

# 1a. Cummulative test number plots

total_c = all_tests_raw %>%
  arrange(RESULT_DATE) %>%
  ungroup() %>%
  summarize(Date_ = unique(RESULT_DATE),
            csum = ecdf(RESULT_DATE)(unique(RESULT_DATE)) * n(),
            test = "Total")


# by test ############################################################
by_test_c = all_tests_raw %>%
  filter(test %in% c("eGFR","HbA1c","TG","TC/HDL ratio")) %>%
  mutate(test = factor(test, levels = c("eGFR","HbA1c","TG","TC/HDL ratio"))) %>%
  arrange(test,RESULT_DATE) %>%
  group_by(test) %>%
  summarize(Date_ = unique(RESULT_DATE),
            csum = ecdf(RESULT_DATE)(unique(RESULT_DATE)) * n() )

cum_df = rbind(total_c,by_test_c) %>%
  mutate(test = factor(test, levels = c("Total","eGFR","HbA1c","TG","TC/HDL ratio"))) 

str(by_test_c)
table(by_test_c$test)

#calculatign final value to use for labeling
final_val_cum_df = cum_df %>%
  group_by(test) %>%
  summarize(Date_ = max(Date_), csum = max(csum)) %>% 
  mutate(test = factor(test, levels = c("Total","eGFR","HbA1c","TG","TC/HDL ratio"))) 

#plotting by test and total
ggplot() +
  geom_line(cum_df, mapping = aes(x = Date_, y = csum, col = test), size = 1.5) +
  geom_text_repel(final_val_cum_df,
                  mapping = aes(x = Date_,y = csum, colour = test, label = test),
                  show.legend = FALSE,
                  min.segment.length= 0,
                  force = 5,
                  direction = "both",
                  nudge_x = 300) + #same units as graph
  labs(
    #title = "Cummulative sum of results by test type",
    x = "Year",
    y = "Cummulative number of results") +
  x_scale_date +
  y_scale +
  scale_color_manual(
    name = "Test type",
    breaks = c("Total","eGFR","HbA1c","TG","TC/HDL ratio"),
...
# save output and clear variables
ggsave(file = "#cumsum_bytest.png",
...
# 2. predictor missingnes exploration

#this is post test safe exclusions
an_cohort = as_tibble(read.fst("data/cohorts/an_cohort.fst"))
...
#group and arrange by VSIMPLE_INDEX_MASTER - takes a while to run
by_id = an_tests %>%
...
temp
temp %>% 
  summarize(all("hba1c" %in% test,"egfr" %in% test,"tchdl" %in% test,"tri" %in% test))
...
cc_plot = ggplot(cc_plot_dt) +
  geom_line(aes(x = id, y = value, colour = variable, group = variable), size = 1.5) + 
...
# 3. predictor distribution - see an_cohort_analysis
...
# #Bind health_contact information and 
# # testsafe_info 
# # HC information is simply repeated for duplicate values
# an_tests = as.tibble(ts_tests) %>%
#   left_join(ts_cohort, by = "VSIMPLE_INDEX_MASTER")



# 1b. incidence plots (not done) ####################
# ts_tests %>%
#   ggplot() +
#   geom_histogram(mapping = aes(x = RESULT_DATE, fill = test),
#                  breaks = x_dates,
#                  boundary = 0,
#                  position = "dodge",
#   ) +
...
#   )) 
# 
# ggsave(file = "ts_incidence_bar.png")
...
# 3. Facility dhb bargraphs ######## 
#dhb by facility
dhb_fac = bound_ts %>%
  group_by(DHB_name,FACILITY_CODE) %>%
  summarize(count = n())

bar_label = dhb_fac %>%
  filter(count > 500000 & count < 5000000)

ggplot(data = dhb_fac, aes(x = DHB_name,y = count, fill = FACILITY_CODE)) +
  geom_col(position = "dodge") +
  geom_label_repel(aes(label = FACILITY_CODE),data = bar_label) +
  labs(
    title = "Raw lab counts per DHB patient",
    x = "DHB",
    y = "Count") +
  y_scale +
  theme_bw()
...
ggplot(data = dhb_fac, aes(x = FACILITY_CODE ,y = count, fill = DHB_name)) +
  geom_col(position = "dodge") +
  labs(
    title = "Raw # of tests",
    x = "Lab code",
...
ggsave("#fac_DHB.png")
#clearing variables
rm(ls = c("total_c","by_test_c","by_fac_c"))
...
ntest_pre_2014 = by_id %>%
  filter(RESULT_DATE < as.Date("2014-01-01") ) %>%
  summarize(
    any_2014 = n(),
    egfr_2014 = sum(test == "egfr"),
    hba1c_2014 = sum(test == "hba1c"),
    tchdl_2014 = sum(test == "tchdl"),
    all_2014 = ("egfr" %in% test & "hba1c" %in% test & "tchdl" %in% test))
...
ntest_pre_2014 #number of tests pre 2014 by id
...
ts_cohort_tests = ts_cohort %>%
  ...
  left_join(ntest_pre_2014, by = "VSIMPLE_INDEX_MASTER") %>%
...
rm( list = c("ntest_","ntest_pre_2014","ntest_pre_2015"))
...
plot_data_ts = ts_age_group %>%
  ...
  summarize(n = n(),
            any = sum(any_ > 0),
            all = sum(all_ > 0),
            any2014 = sum(any_2014 > 0),
            all2014 = sum(all_2014 >0),
